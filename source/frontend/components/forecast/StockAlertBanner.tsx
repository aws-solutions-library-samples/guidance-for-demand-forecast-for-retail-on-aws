...
const alertBannerVariants = cva('relative rounded-lg border-l-4 p-4 transition-all duration-300', {
  variants: {
    severity: {
  ...
  return (
    <div className={cn('space-y-3', className)}>
      {visibleAlerts.map((alert, index) => {
        ...
        const iconColor = alert.severity === 'critical' ? 'text-destructive' : 'text-warning';
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                <h4 className="text-sm font-display font-semibold text-foreground">
                  {alert.severity === 'critical' ? 'Critical Stock Alert' : 'Stock Warning'}
                </h4>
...
export function SingleStockAlertBanner({
  ...
  const sdParts = shortageDate.split('-');
  const formattedDate = new Date(
    parseInt(sdParts[0]),
  ...
  const iconColor = severity === 'critical' ? 'text-destructive' : 'text-warning';
    ...
    >
      <div className="flex items-start gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-foreground mt-1">
            Stock shortage expected on <span className="font-semibold">{formattedDate}</span>.
            {shortageAmount && (
              <>
                {' '}
                Estimated shortage:{' '}
